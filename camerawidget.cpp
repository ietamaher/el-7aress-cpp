...
CameraWidget::CameraWidget(QWidget *parent) : QOpenGLWidget(parent), targetPosition(0, 0)  {
...
void CameraWidget::paintEvent(QPaintEvent *) {
   ...
   //painter.setRenderHint(QPainter::Antialiasing);
    ...
    if (targetPosition != QPoint(0, 0)) {
        painter.drawLine(center, targetPosition);

        // Draw HUD for the selected target
        // For example, draw a circle around the target position
        const int radius = 25;
        const int squareSize = 20;
        QRect squareRect(targetPosition.x() - squareSize / 2, targetPosition.y() - squareSize / 2, squareSize, squareSize);
        painter.translate(targetPosition);
        painter.rotate(45);
        painter.drawRect(squareRect.translated(-targetPosition));
        painter.resetTransform();
        const int crossHairSize = 25;
        const int halfSize = crossHairSize / 2;

        // Horizontal line
        painter.drawLine(targetPosition.x() - halfSize, targetPosition.y(), targetPosition.x() + halfSize, targetPosition.y());
        // Vertical line
        painter.drawLine(targetPosition.x(), targetPosition.y() - halfSize, targetPosition.x(), targetPosition.y() + halfSize);
            
         
        painter.drawEllipse(targetPosition, radius - 5, radius - 5);
